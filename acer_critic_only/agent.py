...
from acer_critic_only.basic_block import DQN
...
class Agent:
    ...
    def set_state_dict(self, new_state_dict):
        for key in self.online_net.state_dict().keys():
            if 'actor' not in key:
                self.online_net.state_dict()[key] = new_state_dict[key]
        return
    ...
    def set_target_dict(self, new_state_dict):
        for key in self.target_net.state_dict().keys():
            if 'actor' not in key:
                self.target_net.state_dict()[key] = new_state_dict[key]
        return
