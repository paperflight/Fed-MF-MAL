...
class DQN(nn.Module):
    ...
    def forward(self, x, actor_or_critic=True, log=False):
        ...
        if actor_or_critic:  # actor run if ture
            p = self.actor_end(x)
            return F.softmax(p, dim=-1), F.log_softmax(p, dim=-1)
        else:  # critic run if false
            v = self.value_end(x)
            return F.softmax(v, dim=-1), F.log_softmax(v, dim=-1)
    ...
    def actor_parameters(self):
        res_tensor = []
        for n, x in self.named_parameters():
            if x.requires_grad and ('actor' in n):
                # since TRPO has NaN problem in CNN gradient BP. Only optimize the policy end.
                res_tensor.append(x)
        return res_tensor
